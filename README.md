...
10. 호스트는 참가자 목록에서 **BTN(버튼) 위치를 지정**할 수 있고, 이를 기준으로 SB/BB/UTG 등 홀덤 포지션이 자동 계산되어 호스트와 각 참가자 화면에 표시됨
11. 참가자 목록은 **드래그 앤 드롭으로 순서 변경** 가능 (테이블 착석 순서를 실제와 맞출 때 사용). 드래그 라이브러리 로드에 실패하면 ▲▼ 버튼으로 대체됨
12. "내보내기"는 각 참가자 옆 **"⋯" 더보기 메뉴** 안으로 이동
